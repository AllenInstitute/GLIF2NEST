...
nest::RecordablesMap< allen::glif_lif_r_asc_a_cond >
  allen::glif_lif_r_asc_a_cond::recordablesMap_;
...
template<>
void
RecordablesMap< allen::glif_lif_r_asc_a_cond >::create()
{
  insert_( names::V_m, &allen::glif_lif_r_asc_a_cond::get_y_elem_< allen::glif_lif_r_asc_a_cond::State_::V_M > );
}
}
...
extern "C" inline int
allen::glif_lif_r_asc_a_cond_dynamics( double,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef allen::glif_lif_r_asc_a_cond::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const allen::glif_lif_r_asc_a_cond& node =
    *( reinterpret_cast< allen::glif_lif_r_asc_a_cond* >( pnode ) );
...
allen::glif_lif_r_asc_a_cond::State_::State_( const State_& s )
{
  y_ = s.y_;
}

allen::glif_lif_r_asc_a_cond::State_& allen::glif_lif_r_asc_a_cond::State_::operator=(
  const State_& s )
{
...
void
allen::glif_lif_r_asc_a_cond::Parameters_::get( DictionaryDatum& d ) const
{
...
void
allen::glif_lif_r_asc_a_cond::Parameters_::set( const DictionaryDatum& d )
{
...
void
allen::glif_lif_r_asc_a_cond::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
...
void
allen::glif_lif_r_asc_a_cond::State_::set( const DictionaryDatum& d,
  const Parameters_& p )
{
...
allen::glif_lif_r_asc_a_cond::Buffers_::Buffers_( const Buffers_& b, glif_lif_r_asc_a_cond& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
allen::glif_lif_r_asc_a_cond::glif_lif_r_asc_a_cond()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
allen::glif_lif_r_asc_a_cond::glif_lif_r_asc_a_cond( const glif_lif_r_asc_a_cond& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
allen::glif_lif_r_asc_a_cond::init_state_( const Node& proto )
{
...
void
allen::glif_lif_r_asc_a_cond::init_buffers_()
{
...
void
allen::glif_lif_r_asc_a_cond::calibrate()
{
  B_.logger_.init();
...
void
allen::glif_lif_r_asc_a_cond::update( Time const& origin, const long from, const long to )
{ 
...
nest::port
allen::glif_lif_r_asc_a_cond::handles_test_event( SpikeEvent&,
  rport receptor_type )
{
  if ( receptor_type <= 0
...
void
allen::glif_lif_r_asc_a_cond::handle( SpikeEvent& e )
{
...
void
allen::glif_lif_r_asc_a_cond::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
allen::glif_lif_r_asc_a_cond::handle( DataLoggingRequest& e )
{
