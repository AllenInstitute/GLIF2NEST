...
Description:
  ...
  and a voltage dependent threshold model) with conductance-based synapses, described in [1].
...
inline size_t
allen::glif_lif_r_asc_a_cond::Parameters_::n_receptors_() const
{
  return tau_syn_.size();
}

inline size_t
allen::glif_lif_r_asc_a_cond::Parameters_::n_ASCurrents_() const
{
  return k_.size();
}

inline nest::port
allen::glif_lif_r_asc_a_cond::send_test_event( nest::Node& target,
...
inline nest::port
allen::glif_lif_r_asc_a_cond::handles_test_event( nest::CurrentEvent&,
  nest::port receptor_type )
{
  if ( receptor_type != 0 )
...
inline nest::port
allen::glif_lif_r_asc_a_cond::handles_test_event( nest::DataLoggingRequest& dlr,
  nest::port receptor_type )
{
  if ( receptor_type != 0 )
    throw nest::UnknownReceptorType( receptor_type, get_name() );
